...
- Present variable template template parameters and concept template parameters
...
Initally proposed in [@P2008R0], variable template template parameters
...
The syntax for these new entities is straigt forward (and live little room for invention)
...
Variable template of a specific types (`template <template <> int>`) are also impossible,
as the type of a template variable can varry accros specializations.
...
complex, repetitive concepts. Barry Revzin offered a number of extremely compeling such use cases
...
parameters are orthoginal features, in that they could be standardized separately.
...
with one another, given the interractions.
...
A universal template parameter can then be:
* A type
* A variable
* A type template
* A variable template
