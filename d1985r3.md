...
automatically marks a dependent name to its left as a type.

This does not change the meaning of `v1`'s declaration, but makes `x1`'s
initializer a value as `type1` is disambiguated by the trailing `::` and
`value1` is a value as there is no leading `typename`. The `x3` and `x4`
initializers behave the same as in C++23, but with a clearer rationale.
...
multiplication. To treat A as a `typename` and a as a variable being declared,
`A` has to be disambiguated, as any dependent name would.
