...
Main discussion of feature in [@P2098R1] by Walter Brown and Bob Steagall.
...
using x = puts_one<takes_var>; // Error! ... behaves covariantly!
...
Let's call the covariant version of `template auto` (with the meaning "deduce
this from the argument") `auto template`:
...
This would work *contravariantly*! While `template auto ARG` means "deduce",
a function taking a concept (which behaves contravariantly!) that *anything*
satisfies *has* to be a function taking `template auto` (or `std::anything`).
...
will know to use a library-provided concept.
