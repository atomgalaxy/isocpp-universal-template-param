...
but we can still constrain with 
...
- metafunction parameter constraints:
  ...
  - constraint relaxation (means "no constraint in this direction"):
    - `...` meaning "no constraint on arity"
    ```cpp
    ...
    ```
    - `__` meaning "no constraint on kind"
    - "no constraint on type" - covariant `auto`, no proposed spelling
