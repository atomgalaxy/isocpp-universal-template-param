...
To understand covariance and contravariance, one needs to look at the conversion rules for function pointers.

`auto` template parameters, by the standard, behave as one would expect.

```cpp
template <auto> struct takes_auto {};
template <int> struct takes_int {};
template <template <int> typename accepts_int> struct takes_function_of_int {
  using type = accepts_int<1>;
};
template <template <auto> typename accepts_auto> struct takes_function_of_auto {
  using type1 = accepts_auto<1>;
  using type2 = accepts_auto<nullptr>;
};
takes_function_of_int<takes_auto> x{}; // ok
takes_function_of_int<takes_int> y{}; // ok
takes_function_of_auto<takes_auto> z{}; // ok
takes_function_of_auto<takes_int> w{}; // erro
```

The error source should be obvious - `takes_function_of_auto` can't accept a function that doesn't take *any* non-type template parameter.

However, that's currently not what happens in MSVC, clang and gcc (trunk) - the error one gets is versions of "could not convert 'nullptr' from 'std::nullptr_t' to 'int'".
