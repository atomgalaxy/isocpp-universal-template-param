...
template<template<typename> class PRED, size_t POS, typename TL> 
constexpr size_t tuple_find()
{
    if constexpr (POS == tuple_size<TL>()) {
        return npos;
    }
    else if constexpr (PRED<remove_cvref_t<tuple_element_t<POS, TL>>>::value)
        return POS;
    else
        return tuple_find<PRED, POS + 1, TL>();
}
template<template<typename> class PRED, typename TL> 
constexpr size_t tuple_find() { return tuple_find<PRED, 0, TL>(); }

// Helper to bind the first arguments of a provided template
template<template<typename auto...> TPL, template auto... BS> struct {
    template<typename auto... Ts> using func = TPL<BS..., Ts>;
};
...
template<template<typename> class PRED, size_t POS, typename TL>
constexpr size_t tuple_find_v<PRED, TL> = tuple_find<PRED, POS, TL>();
...
constexpr size_t tuple_find_v<PRED, M, TL> = tuple_find_v<curry<PRED, M>::template func, TL>;
...
based only on its type. Unfortunately today `tuple_find` can only be
...
previous example instead of as the function it must be now.
