...
## Contravariance
```cpp
template <template <B> typename f> using puts_b = void;
template <template <A> typename f> using puts_a = void;
template <B> using takes_b = void;
template <A> using takes_a = void;
using x = puts_b<takes_a>; // ok
// Error, constraint mismatch (gcc, clang accepts in error)
...
_more general_ function than one that just accepts `B`s.
...
Let's just replace `is_a` with `auto` and `is_b` with `int`:

```cpp
template <template <int> typename f> using puts_b = void;
template <template <auto> typename f> using puts_a = void;
template <int> using takes_b = void;
template <auto> using takes_a = void;
using x = puts_b<takes_a>; // OK
using w = puts_a<takes_b>; // OK??? MSVC, GCC and clang all accept
```

both co- and contravariance of function pointers is disallowed completely,
except for virtual functions, where (pointer and reference) covariant return
types are allowed, but there are no contravariant parameters.

How about parameter packs?

```cpp
template <template <typename> typename f> using puts_b = void;
template <template <typename...> typename f> using puts_a = void;
template <typename> using takes_b = void;
template <typename...> using takes_a = void;
using x = puts_b<takes_a>; // Error! Behaves covariantly!
using w = puts_a<takes_b>; // OK
```

Turns out parameter packs behave *covariantly* - exactly the opposite of what
one would expect.
...
```cpp
template <template <auto> typename f> using puts_b = void;
template <template <template auto> typename f> using puts_a = void;
template <auto> using takes_b = void;
template <template auto> using takes_a = void;
using x = puts_b<takes_a>; // OK
using w = puts_a<takes_b>; // Error because contravariant.
```
...
The above is correct, but useless - it requires `f`s signature to be
`template <template auto T>`. What we actually want to express is that `f` is
a unary template template, so we can pass something like 
`template <int x> using int_constant = std::integral_constant<int, x>;` to
it.

Perhaps a good spelling of "deduce this from the argument" is
`auto template`:
...
This is what we want to express (and check at instantiation time).

This requires us to land-grab an additional combination of tokens. Contrast
what happens if we just made `template auto` behave covariantly (the way
`auto template` behaves above) if used in a template-template parameter list.
What do we lose?
...
This would work because while `template auto ARG` means "deduce", a function
taking a concept (concepts behave contravariantly!) that *anything* satisfies
*has* to be a function taking `template auto` (or `std::anything` for an exact
match)!

In light of this, the paper authors have come to the conclusion that the
example to follow with `template auto` should be the `...` (covariant,
deduction behavior).

It means that usage is the same as declaration, and like binds with like. We
anticipate the feature being much easier to teach this way, and in the rare,
rare cases when someone will really want to take a template that can process
anything, they will know to use a library-provided concept.
