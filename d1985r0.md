...
struct alias<Arg> {
  template <template auto... As> using value = Arg<As...>;
};
```
...
class templates by implementing each overload as a specialization of a
unimplemented primary template. As this is not real overloading there will be some
differences, especially when it comes to CTAD.
...
select the first specialization unless an allocator object is given
in which case the second specialization is selected. To select the
third specialization the template arguments must be explicitly given.


## Overloading of variable templates

Universal template parameters can be used to implement what appears like overloading a variable
template. The problem of not being able to delete the base case then becomes more
pressing than currently as selecting the base case should trigger an error but
will be unconditionally selected when none of the specialziations matches. The dependent
static_assert mentioned above can be used to rescue this situation but
this is still somewhat of a hack.

```cpp
template<template auto... Ps>
constexpr auto tuple_find_v = [] { static_assert(dependent_false_v<Ps...>); };

template<template<typename> class PRED, typename TL>
constexpr size_t tuple_find_v<PRED, TL> = tuple_find<PRED, TL>();

template<template<typename, typename> class PRED, typename M, typename TL> 
constexpr size_t tuple_find_v<PRED, M, TL> = tuple_find_v<metafunction_curry<PRED, M>, TL>;

template<typename T, typename TL>
constexpr size_t tuple_find_v<T, TL> = tuple_find_v<std::is_same, T, TL>;

```

This example is a metafunction to find a matching element in a tuple
based only on its type. The first overload is usable for a unary predicate like
std::is_reference while the second overload is useful for a binary
predicate like std::is_base_of by using a currying helper.

Currently this can't be implemented as a variable template which
breaks the symmetry with simpler tuple oriented metafunctions such as
tuple_size_v and tuple_element_t.

Given an overloaded constexpr function tuple_size we could instead
simplify this to:
...
relying on the power of universal template parameters in another way.
This has the same simplicty as the current variable templates and type
aliases of the standard library type traits.

To take the consistency a step further tuple_find could be implemented
as a class template with universal template parameters as shown in the previous example
instead of as the function it must be now.
