...
This paper aims to generalize template parameters to completeness (closure, in the topological sense).
...
concepts). This model is not uniform in C++23 because C++23 lacks a way to treat
...
### Contrast with Reflection

The authors would like to thank Daveed Vandevoorde for this example.

With [@P1240R2], the implementation would be like so:

```cpp
template<typename Type, template <template auto...> typename Templ>
...
```cpp
consteval bool is_specialization_of(std::meta::info r_instance, std::meta::info r_templ) {
...
template <template auto> constexpr bool is_class_template_v = false;
...
// As we propose universal template template-parameters
template <template auto> constexpr bool is_template_v       = false;
template <template <template auto...> template auto A>
constexpr bool is_template_v<A>                             = true;

// The associated type for each trait:
template <template auto X> struct is_typename
    : std::bool_constant<is_typename_v<X>> {};
template <template auto X> struct is_value
    : std::bool_constant<is_value_v<X>> {};
template <template auto X> struct is_class_template
    : std::bool_constant<is_class_template_v<X>> {};
...
template <template auto X> struct is_template
    : std::bool_constant<is_template_v<X>> {};
```
...
version still needs box for the result of `Reduce` as it does not rely on _universal
aliases_.
...
### Manual disambiguation to template template

(Example modified from one provided by Tomasz Kamiński)

Given:

```cpp
template <template auto Y>
template auto X = Y;

template <template auto U>
void f() {
   // expect U<X> is a variable-template and U is a template
   return template (template U<X>)<2>; // proposed valid
   //               ~~~~~~~~~~~~~      <- expression of deferred kind #1
   //     ~~~~~~~~~~~~~~~~~~~~~~~~~~~  <- expression of deferred kind #2
}
```

In this (admittedly, probably rare) case, we have a template-head following some token of deferred kind, which requires us to tell the parser to treat the template-head as a template-head.

It is debatable whether we should require parentheses there, or whether `template template U<X><2>` would be fine. The authors see no reason to require parentheses.
...
some additional disambiguator. More work would be needed to establish the feasibility
...
especially for dependant expressions (Even if neither variable or concept template template parameters
should not require new disambiguators, `template` is enough to disambiguate all types
...
the preferred new keyword spelling recommended from EWG(I?).
...
The examples in this paper are pending discussion with reflection authors.
...
Thanks to Tomasz Kamiński for finding the double-template example.
